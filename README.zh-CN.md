...
| 模块 | 能力 |
| --- | --- |
| 运行时管理 | 安装并切换运行时版本——当前支持 PHP、Nginx、MySQL、Java、Node.js、Go，后续还会持续增加。 |
| 服务运维 | 启动、停止、重启本地服务，统一查看状态、端口和日志。 |
| 配置与站点 | 编辑 `php.ini`、`nginx.conf`、虚拟主机、`my.cnf`，管理 hosts 记录和 MySQL。 |
| 工具链能力 | Composer、npm/Corepack、Go 环境工具一站化管理。 |
...
- 通过可插拔的 provider 体系安装并管理多个运行时版本——当前支持 PHP、Nginx、MySQL、Java、Node.js 和 Go，后续会持续增加更多运行时。
...
- 通过可插拔 provider 体系的运行时管理（目前含 PHP、Nginx、MySQL、Java、Node.js、Go）。
