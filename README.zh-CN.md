...
Envora 是一个面向 PHP 开发者的桌面端本地开发环境管理工具。

它帮助你在一个友好的界面中安装和切换运行时、启动或停止服务、编辑常用配置文件、管理本地域名站点，并把 Composer 放在触手可及的位置。

> Envora 正在走向开源。项目还很年轻，但方向很明确：让本地 PHP 开发环境更轻、更可见，也更容易被协作和分享。

## Envora 能做什么

- 在桌面应用中管理 PHP、Nginx 和 MySQL 运行环境。
- 安装支持的运行时版本，并显示下载和安装进度。
- 设置默认 PHP 版本，并将运行时命令暴露到 Shell 环境。
- 启动、停止、重启服务，并查看服务状态。
- 查看和清空服务日志。
...
Envora 希望把这些状态变得清晰可见。它不是要隐藏 PHP、Nginx、MySQL 或 Composer，而是把关键控制集中到一个地方，同时保留配置文件可编辑、运行时可理解、问题可排查的开发体验。

## 当前状态

Envora 目前处于早期开发阶段。

应用中已经包含运行时管理、服务控制、配置编辑、虚拟主机、MySQL、Composer 和设置等页面，但它还不是一个完全稳定的正式版本。部分能力可能仍然以 macOS 为主，跨平台支持和发布流程也还在继续完善。
...
│       ├── core/         # 平台工具和共享错误
│       ├── download/     # 下载和解压逻辑
│       ├── runtime/      # PHP、Nginx、MySQL provider
│       ├── service/      # 服务生命周期管理
│       └── settings/     # 应用设置和路径
└── docs/                 # 项目文档
```
